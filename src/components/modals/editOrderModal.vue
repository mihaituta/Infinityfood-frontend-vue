<template>
    <v-layout class="pa-0 ma-0" max-width="25%">
        <v-dialog v-model="openModal" max-width="15%" full-height transition="scale-transition">
            <v-card>
                ...
                <v-form ref="form">
                    <v-card-text class="mt-0 pt-0 pb-0 mb-0" style="padding-left: 20px;padding-right: 20px;">
                        <v-select :items=statusList item-text="text" item-value="value" label="Status" v-model="status"
                            box>
                    ...
                    <v-card-actions class="pb-3 pt-0 mt-0 justify-center">
                        ...
                        <v-btn color="primary" :disabled="loading" @click.prevent="onSubmit">Update</v-btn>
                    </v-card-actions>
                </v-form>
            </v-card>
        </v-dialog>
