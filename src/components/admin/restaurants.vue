    ...
    <div class="wrapper">
        <v-container fluid style="width:95%">
            <notification text="Restaurant was deleted successfully!" color="rgb(255, 82, 82, 0.9)" :top=true
                :right=true :showNotification="restaurantDeletedNotif"></notification>
            ...
            <v-card class="mt-4 mb-2">
                ...
                <v-data-table v-if="restaurants" :headers="headers" :items="restaurants" :search="search"
                    :expand="expand" class="elevation-1">
                    <template slot="items" slot-scope="restaurant">
                        <tr @click="restaurant.expanded = !restaurant.expanded">
                            ...
                            <td>{{ users.find(e => e.id === restaurant.item.userId).name }}</td>
                            ...
                            <v-layout class="justify-space-around align-center mt-3 mb-3">
                                ...
                                <v-flex xs2>
                                    <v-tooltip fixed top max-width="400px" color="white">
                                        <template v-slot:activator="{ on }">
                                            <v-img :src="path + restaurant.item.logoImage" aspect-ratio="1" height="170"
                                                contain v-on="on"></v-img>
                                        </template>
                                ...
                                <v-flex xs4>
                                    <v-tooltip fixed top max-width="1000px" color="white">
                                        <template v-slot:activator="{ on }">
                                            <v-img :src="path + restaurant.item.backgroundImage" aspect-ratio="1"
                                                height="190" contain v-on="on"></v-img>
                                        </template>
                                ...
                                <v-flex xs4>
                                    <v-tooltip fixed top max-width="800px" color="white">
                                        <template v-slot:activator="{ on }">
                                            <v-img :src="path + restaurant.item.previewImage" aspect-ratio="1"
                                                height="190" contain v-on="on"></v-img>
                                        </template>
