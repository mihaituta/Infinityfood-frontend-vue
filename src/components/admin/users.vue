<template>
    <div class="wrapper">
        <v-container fluid style="width:80%">
            <notification text="User was deleted successfully!" color="rgb(255, 82, 82, 0.9)" :top=true :right=true
            ...
            <v-card class="mt-4 mb-3">
                ...
                <v-data-table v-if="users" :headers="headers" :items="users" :search="search" class="elevation-1">
                    <template slot="items" slot-scope="user">
                        <td>{{ user.item.id }}</td>
                        <td>{{ user.item.name }}</td>
                        <td>{{ user.item.email }}</td>
                        <td>{{ user.item.roleId }}</td>
